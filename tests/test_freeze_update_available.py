...
def get_current_filename(system):
    return VERSIONS["updates"][APP_NAME][CURRENT_VERSION_PYU_FORMAT][system]["filename"]


def get_patch_filename(system):
    return VERSIONS["updates"][APP_NAME][UPDATE_VERSION_PYU_FORMAT][system][
        "patch_name"
    ]


def get_update_filename(system):
    return VERSIONS["updates"][APP_NAME][UPDATE_VERSION_PYU_FORMAT][system]["filename"]
...
class FreezeUpdateAvailableTester(unittest.TestCase):
    ...
    def setUp(self):
        ...
        current_filename = get_current_filename(self.system)

        update_filename = get_update_filename(self.system)
        patch_filename = get_patch_filename(self.system)
        ...
        VERSIONS["updates"][APP_NAME][CURRENT_VERSION_PYU_FORMAT][self.system][
            "file_hash"
        ...
        server_update_file_path = os.path.join(self.server_dir_path, update_filename)
        ...
        VERSIONS["updates"][APP_NAME][UPDATE_VERSION_PYU_FORMAT][self.system][
            "file_hash"
        ...
        server_keys_file_path = os.path.join(
            self.server_dir_path, settings.KEY_FILE_FILENAME
        )
        ...
        temp_file = tempfile.NamedTemporaryFile()
        self.py_updater_dir_path = temp_file.name
        temp_file.close()
        # Hacking setting with absolute path to have pyBuilder using this folders.
        settings.CONFIG_DATA_FOLDER = os.path.join(
            self.py_updater_dir_path, settings.CONFIG_DATA_FOLDER
        )
        settings.USER_DATA_FOLDER = os.path.join(
            self.py_updater_dir_path, settings.USER_DATA_FOLDER
        )
        os.mkdir(self.py_updater_dir_path)
    ...
    def test_freeze_update_available(self):
        ...
        build_filename = "%s-%s-%s%s" % (APP_NAME, self.system, CURRENT_VERSION, ext)
        ...
        os.remove(os.path.join(self.server_dir_path, get_update_filename(self.system)))
    ...
    def tearDown(self):
        ...
        try:
            shutil.rmtree(self.py_updater_dir_path)
        except OSError:
            logger.warning("Couldn't remove %s", self.py_updater_dir_path)
        os.chdir(self.run_py_file_path)
        try:
